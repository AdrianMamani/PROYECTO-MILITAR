...
class GaleriaPublicaController {
    ...
    public function mostrarGaleria() {
        ...
        require_once './views/galeria.php'; // Recibe: $imagenes, $logros, $videosLogro
    }
    ...
    public function verLogro($id) {
        ...
        if ($logro) {
            ...
            $todasImagenes = $this->modeloImagenLogro->getAll();

            // Filtrar imágenes de este logro
            $imagenesLogro = array_values(array_filter($todasImagenes, function($img) use ($id) {
                return $img['logro_id'] == $id;
            }));

            // Obtener la última imagen como imagen principal
            $ultimaImagen = end($imagenesLogro);
            $imagenPrincipal = $ultimaImagen ? '../uploads/logros/' . $ultimaImagen['nombre_imagen'] : '../assets/img/default-logro.jpg';

            // Eliminar la última imagen del array para galería
            array_pop($imagenesLogro);

            // Galería en orden inverso
            $galeriaImagenes = array_reverse($imagenesLogro);

            // Obtener videos de este logro
            $videosLogro = array_filter($this->modeloVideoLogro->getAll(), function($vid) use ($id) {
                return $vid['logro_id'] == $id;
            });

            // Incluir vista adaptada para mostrar un solo logro con sus medios
            require_once './views/galeria_id.php';
        } else {
  ...
  <style>
   body {
    ...
    .carousel-button {
      ...
      background-color: rgba(8, 19, 179, 0.8);
      color: white;
      border: none;
    ...
    .carousel-button:disabled {
      background-color: rgba(147, 148, 255, 0.4);
      cursor: default;
    }
    ...
    ul.comment-list li img.avatar {
      ...
      object-fit: cover;
      flex-shrink: 0;
      box-shadow: 0 0 0 2px rgb(13, 0, 255);
    }
    .comment-content {
...
<?php foreach ($logros as $logro): ?>
  <header>
    ...
    <small><?= htmlspecialchars($logro['fecha']) ?></small>
