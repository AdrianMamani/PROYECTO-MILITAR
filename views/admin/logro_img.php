...
<div class="wrapper">
                ...
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
                    ...
                    <li class="nav-item has-treeview">
                        ...
                        <ul class="nav nav-treeview">
                            ...
                            <li class="nav-item">
                                <a href="index.php?action=logroimg" class="nav-link active">
                                    <i class="far fa-circle nav-icon"></i>
                                    <p>Imágenes</p>
                                </a>
                            </li>
    ...
    <tbody>
        <?php if (empty($imagenes)): ?>
            <tr><td colspan="4">No hay imágenes de logros especiales</td></tr>
        <?php else: ?>
            <?php foreach ($imagenes as $imagen): ?>
                <tr>
                    <td><?= htmlspecialchars($imagen['id']) ?></td>
                    <td>
                        <img src="/PROYECTO-MILITAR/uploads/logros/<?= htmlspecialchars($imagen['nombre_imagen']) ?>" 
                    ...
                    <td><?= htmlspecialchars($imagen['titulo_logro']); ?></td>
                    <td>
                        <button type="button" class="btn btn-warning btn-sm edit-button" 
                            data-id="<?= htmlspecialchars($imagen['id']) ?>" 
                            data-imagen="/PROYECTO-MILITAR/uploads/logros/<?= htmlspecialchars($imagen['nombre_imagen']) ?>"
                            data-logro-id="<?= htmlspecialchars($imagen['logro_id']) ?>"
                            data-logro-nombre="<?= htmlspecialchars($imagen['titulo_logro']) ?>"
                            data-toggle="modal" data-target="#editModal">
                            <i class="fas fa-edit"></i> Editar
                        </button>
                        <a href="index.php?action=logroimg/delete&id=<?= $imagen['id'] ?>" class="btn btn-danger btn-sm" 
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="addForm" action="index.php?action=logroimg/store" method="POST" enctype="multipart/form-data">
        <div class="modal-body">
          <div class="form-group">
            <label for="imagen">Imagen</label>
            <input type="file" class="form-control-file" id="imagen" name="imagen" required>
            <small class="form-text text-muted">Seleccione la imagen para el logro especial.</small>
            <div class="mt-2">
              <img id="preview-add" src="" alt="Vista previa" class="img-fluid d-none" style="max-height: 200px;">
            </div>
          </div>
          <div class="form-group">
            <label for="logro_id">Logro Especial</label>
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">Editar Imagen del Logro Especial</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Cerrar">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="editForm" method="POST" action="index.php?action=logroimg/update" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <small class="form-text text-muted">Seleccione la nueva imagen para el logro especial.</small>
          </div>
          <div class="form-group">
            <label for="edit-logro_nombre">Logro Especial</label>
            <input type="text" class="form-control" id="edit-logro_nombre" name="logro_nombre" readonly>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
...
<script>
  ...
  $('#editModal').on('show.bs.modal', function (event) {
    ...
    var logroNombre = button.data('logro-nombre');

    var modal = $(this);
    modal.find('#edit-id').val(id);
    modal.find('#edit-logro_nombre').val(logroNombre);
  ...
  $('#addForm').submit(function (e) {
    ...
    $.ajax({
      url: 'index.php?action=logroimg/store',
      method: 'POST',
      data: formData,
      contentType: false,
      processData: false,
  ...
  $('#viewImageModal').on('show.bs.modal', function (event) {
    ...
    var imagen = button.data('imagen');
    var modal = $(this);
    modal.find('#full-image').attr('src', '/PROYECTO-MILITAR/uploads/logros/' + imagen);
  });
