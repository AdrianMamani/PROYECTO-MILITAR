...
<body class="hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
                    ...
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
                        ...
                        <li class="nav-item has-treeview">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                <li class="nav-item">
                                    <a href="index.php?action=emprendimiento_videos" class="nav-link">
                                        <i class="far fa-circle nav-icon"></i>
                                        <p>Videos</p>
                                    </a>
                                </li>
                        ...
                        <li class="nav-item has-treeview">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                <li class="nav-item">
                                    <a href="index.php?action=usuarios_imagenes" class="nav-link">
                                        <i class="far fa-circle nav-icon"></i>
                                        <p>Imágenes</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="index.php?action=usuarios_videos" class="nav-link">
                                        <i class="far fa-circle nav-icon"></i>
                                        <p>Videos</p>
                                    </a>
                                </li>
                                    ...
                                    <?php if (!empty($imagenes)): ?>
                                        <?php foreach ($imagenes as $imagen): ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($imagen['nombre_usuario']) ?></td>
                                                <td><?= htmlspecialchars($imagen['fecha_subida']) ?></td>
                                                <td>
                                                    <img src="/PROYECTO-MILITAR/uploads/usuarios/imagenes/<?= htmlspecialchars($imagen['nombre_imagen']) ?>"
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-sm btn-warning edit-imagen"
                                                        data-id="<?= $imagen['id'] ?>"
                                                        data-correo="<?= htmlspecialchars($imagen['nombre_imagen']) ?>"
                                                        data-imagen="<?= htmlspecialchars($imagen['nombre_imagen']) ?>"
                                                        data-toggle="modal"
                                                        data-target="#editarImagen">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <a href="index.php?action=usuarios_imagenes/eliminar/<?= $imagen['id'] ?>"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addModalLabel">Agregar Imagen</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="index.php?action=usuarios_imagenes/agregar" method="POST" enctype="multipart/form-data">
        ...
        <!--editar-->
        <div class="modal fade" id="editarImagen" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form id="editForm" method="POST" action="index.php?action=usuarios_imagenes/editar" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="imagen">Imagen</label>
                                <input type="file" class="form-control" id="imagen_edit" name="imagen_edit" required>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
    ...
    <script>
        $(document).ready(function() {
            $('.edit-imagen').on('click', function() {
                $('#edit-id').val($(this).data('id'));
                $('#editForm').attr('action', 'index.php?action=usuarios_imagenes/editar/' + $(this).data('id'));
            });
        });
        $('#viewImageModal').on('show.bs.modal', function(event) {
            ...
            var modal = $(this);
            modal.find('#full-image').attr('src', '/PROYECTO-MILITAR/uploads/usuarios/imagenes/' + imagen);
        });
    </script>
</body>
