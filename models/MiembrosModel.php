...
{
    private $db;
    private $nombreTabla = 'usuarios'; // Nombre de la tabla actualizado
    ...
    {
        $query = "INSERT INTO usuarios (nombre, descripcion, especialidad_id, imagen) 
                    VALUES (?, ?, ?, ?)";
        $stmt = $this->db->prepare($query);
    ...
    {
        $sql = "UPDATE usuarios SET nombre = ?, descripcion = ?, especialidad_id = ?, imagen = ? WHERE id = ?";
        $stmt = $this->db->prepare($sql);
